...
def cp_model(device,train_ds,valid_ds):
    batch_size = 32
...
def train_cp_model(cp_model,epoch,device):
    model = cp_model["model"]
    train_loader = cp_model["train_loader"]
    valid_loader = cp_model["valid_loader"]
    optimizer = cp_model["optimizer"]
    min_test_mae = cp_model["min_test_mae"]
    ...
    if not is_nan_test and not is_nan_train:
        ...
        if test_mae < min_test_mae:
            cp_model["best_valid_model"] = model_copy
            cp_model["min_test_mae"] = test_mae
    else:
        cp_model["nan"] = True
# def train_cp_model(train_ds, valid_ds, fold,device):
#     batch_size = 32
#     model_dir = os.path.abspath('./model/'+str(fold))
#     # print('train_ds size = {}, test_ds size = {}'.format(len(train_ds), len(valid_ds)))
# 
#     train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, drop_last=True)
#     test_loader = DataLoader(valid_ds, batch_size=batch_size, shuffle=True, drop_last=True)
# 
#     data_ini = None
#     for step, data in enumerate(train_loader):
#         if step == 0:
...
# 
#     min_train_mape = 1000000000
#     min_test_mape = 1000000000
#     best_valid_model = None
#     for epoch in range(500):
#         train_loss, train_mape,is_nan_train = train(model, train_loader,device,optimizer)
#         test_loss, test_mape,is_nan_test = test(model, test_loader, epoch,device)
...
# 
#         if save_train:
#             torch.save(checkpoint_1, os.path.join(model_dir, 'hier_'+target[tar_idx] + '_mean_h64_d0_checkpoint_train.pt'))
# 
#         if save_test:
#             torch.save(checkpoint_2, os.path.join(model_dir, 'hier_'+target[tar_idx] + '_mean_h64_d0_checkpoint_test.pt'))
