...
# def train_pwr_model(train_ds, valid_ds, fold,device):
#     batch_size = 32
...
#     return min_train_mape,min_test_mape,best_valid_model
def pwr_model(device,train_ds,valid_ds):
    batch_size = 32
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True, drop_last=True)
    valid_loader = DataLoader(valid_ds, batch_size=batch_size, shuffle=True, drop_last=True)
...
def train_pwr_model(pwr_model,epoch,device):
    model = pwr_model["model"]
    train_loader = pwr_model["train_loader"]
    valid_loader = pwr_model["valid_loader"]
    optimizer = pwr_model["optimizer"]
    min_test_mae = pwr_model["min_test_mae"]
    ...
    if not is_nan_test and not is_nan_train:
        ...
        if test_mae < min_test_mae:
            pwr_model["best_valid_model"] = model_copy
            pwr_model["min_test_mae"] = test_mae
    else:
        pwr_model["nan"] = True
