...
from hgp.ironman_pro.graph_model_dsp import *
from hgp.ironman_pro.graph_model_bram import *
from hgp.ironman_pro.graph_model_cp import *
from hgp.ironman_pro.graph_model_ff import *
from hgp.ironman_pro.graph_model_pwr import *
from hgp.ironman_pro.graph_model_lut import *
import torch
...
def pred_obj(models,data,device):
    ...
    for obj_type, model in models.items():
        ...
        with torch.no_grad():
            data = data.to(device)
            if obj_type == "cp":
                out = model(data.cp_x, data.cp_edge_index, data.batch)
            else:
...
def save(reg,epoch,model_dir,type):
    ...
    torch.save(checkpoint, os.path.join(model_dir, 'iron_' + type + '_test.pt'))
def train_iron_gnns(train_ds,valid_ds,train_ds_cp,valid_ds_cp,siamese_test,fold):
    epoches = 500
    ...
    for epoch in range(epoches):
        ...
        models = {"bram": bram["best_valid_model"], "ff": ff["best_valid_model"], "dsp": dsp["best_valid_model"], "lut": lut["best_valid_model"],
                  "cp": cp["best_valid_model"] ,"pwr": pwr["best_valid_model"]}
...
for i in range(k):
    ...
    max_valid_acc = train_iron_gnns(k_train[i], k_test[i],k_train_cp[i],k_test_cp[i],siamese_test[i],i+1)
    logs["acc"].append(max_valid_acc)

    print("\033[32m ACC: %f\033[0m" % max_valid_acc)
np.save("./k_result/"+model_name+".npy", np.array(logs))
